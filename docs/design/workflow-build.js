...
const COMMON = `
PROJECT ROOT: ${REPO} (a standalone repo — NOT Travelio; never touch ~/Desktop/travelio-asim-shadow). Read ${REPO}/CLAUDE.md, then ${DESIGN}/SPEC.md (the contract), then ${DESIGN}/HEAD-RULINGS.md in full — Part 1 and Part 2 OVERRIDE SPEC wherever they conflict (Part 2 moves everything to this repo root: SPEC's tools/llm-detect/X is ${REPO}/X, SPEC's .claude/agents file is ${REPO}/agent/llm-text-detector.md, own_bot_marker is the generic known_machine_marker with markers.json, the corpus pull is eval/adapters/supabase-messages.cjs, the agent finds the tool via LLM_DETECT_HOME).
...
STANDING RULES: zero paid API calls; network only in eval/fetch-public-datasets.mjs against datasets-server.huggingface.co (+ the single ALHD Zenodo attempt of R9); no git commands that change state (no add/commit/stash/checkout/reset); touch ONLY the files your builder id owns (SPEC §I mapped by R16); scratch under ${SCR}/<your-id>/; Node v24.5.0, ESM, zero runtime dependencies in the shipped tool; language scope EN/TR/AR; Arabic literals in code/lexicons stored \\uXXXX-escaped and never printed to the terminal for review (diff programmatically).
...
Before finishing: run SPEC §I acceptance checks 1–7 (paths relative to ${REPO}) and paste observed output into 'verified'; then run the CLI on 10 varied inputs of your own (EN/TR/AR, chat and prose, a leaked-assistant text, a text containing a marker you put in a temporary markers file, a formal non-native email) and include a verdict table in your summary confirming §D.3 behaviour.`
...
3. RUBRIC.md: D2's C01–C20 with FP traps and the banned-inference list copied (not paraphrased) from ${PRIVATE}/D2-judge-agent.md — but NO private corpus rows may be copied into this public file; SPEC §E.1 table + four invariants, §E.2, §E.3 skeleton + Arabic reporting rule, §E.4 signal-name map verbatim incl. the no-CLI-counterpart list, and R1–R3/R17 as they change what a rule means.
