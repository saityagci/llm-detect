...
const COMMON = `
PROJECT ROOT: ${REPO} (a standalone repo — NOT Travelio; never touch ~/Desktop/travelio-asim-shadow). Read ${REPO}/CLAUDE.md, then ${DESIGN}/SPEC.md (the contract), then ${DESIGN}/HEAD-RULINGS.md in full — Part 1, Part 2 and R22 (ARABIC OUT) OVERRIDE SPEC wherever they conflict (Part 2 moves everything to this repo root: SPEC's tools/llm-detect/X is ${REPO}/X, SPEC's .claude/agents file is ${REPO}/agent/llm-text-detector.md, own_bot_marker is the generic known_machine_marker with markers.json, the corpus pull is eval/adapters/supabase-messages.cjs, the agent finds the tool via LLM_DETECT_HOME).
