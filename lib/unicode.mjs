...
/** 1:1 per code point, so `foldConfusables(s).length === s.length` and spans line up exactly. */
export function foldConfusables(s) {
  let out = '';
  let changed = false;
  for (const ch of s) {
    const cp = ch.codePointAt(0);
    let r = CONFUSABLE_MAP.get(ch);
    if (r === undefined && cp >= FULLWIDTH_LO && cp <= FULLWIDTH_HI) {
      r = String.fromCodePoint(cp - FULLWIDTH_DELTA);
    }
...
/**
 * homoglyphScan(tokens) -> { count, examples } (R30).
 * A word token is suspect when its letters come from more than one script, or when it carries a
 * Halfwidth/Fullwidth or Mathematical-Alphanumeric code point.
 */
