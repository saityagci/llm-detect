...
import { makeViews, foldConfusables, homoglyphScan } from './unicode.mjs';
...
function buildContext(text, opts) {
  ...
  ctx.homoglyphs = homoglyphScan(toks);
  ...
  if (MATCHER && (lang === 'en' || lang === 'tr')) {
    ...
    const foldedConf = foldConfusables(ctx.foldedLex);
    if (foldedConf !== ctx.foldedLex) {
      const seen = new Set(raw.map((h) => `${h.start}:${h.end}:${h.id}`));
      for (const h of search(MATCHER, foldedConf)) {
        const k = `${h.start}:${h.end}:${h.id}`;
        if (!seen.has(k)) { seen.add(k); raw.push(h); }
      }
...
export function detect(text, opts = {}) {
  ...
  const cell = weights.cells[key];
  if (!cell) throw new Error(`weights file has no cell "${key}"`);
...
export function aggregate(messages, opts = {}) {
  ...
  const weights = opts.weights ?? WEIGHTS;
  const cell = weights.cells[base.scoring.cell];
  ...
  return {
    ...
    warnings: [...new Set([...base.warnings, ...decision.warnings, ...guard.warnings])],
