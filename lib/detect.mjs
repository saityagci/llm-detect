...
import { compareHistory, historySignal, normalizeHistory, HISTORY_MIN_TOKENS } from './history.mjs';
import {
...
export function buildSummary(verdict, rules, gateReason) {
  const label = SUMMARY_LABELS[verdict] ?? 'no_reliable_indicators';
  const matched = label === 'fingerprint_found'
...
export function detect(text, rawOpts = {}) {
  ...
  if (Array.isArray(opts.history) && opts.history.length > 0) {
    ...
    if (historySkipped.length) {
      notes.push(`history_rows_skipped: ${historySkipped.length} prior row(s) carried no string `
        + `"text" and were skipped (index ${historySkipped.join(', ')}). A history row is either a `
        + 'string or an object with a string `text` field.');
    }
    const priorDocs = historyTexts.map((t) => {
      const r = detect(t, { ...opts, history: undefined, id: undefined, mode: 'single' });
      return { tokens: r.counts.tokens, signals: r.signals.filter((x) => x.value !== null) };
    });
    const cmp = compareHistory(priorDocs, active);
    historyBlock = {
      priorDocs: cmp.priorDocs,
      comparedFeatures: cmp.comparedFeatures,
      shifted: cmp.shifted,
      minTokensPerDoc: HISTORY_MIN_TOKENS,
      ...(historySkipped.length ? { skippedRows: historySkipped } : {}),
    };
    if (cmp.insufficient) {
      warnings.push('history_insufficient');
      notes.push(`history_insufficient: ${cmp.usableDocs} of ${historyTexts.length} prior `
      ...
      notes.push(`consistent_with_history: this submission sits within 2 SD of the author's own `
        + `prior ${cmp.priorDocs} submission(s) on ${cmp.comparedFeatures} compared feature(s). `
  ...
  const report = {
    ...
    verdict,
    summary: buildSummary(verdict, ruleOut.rules, reason),
...
export function aggregate(messages, rawAggOpts = {}) {
  ...
  return {
    ...
    messageCount: messages.length,
    verdict: guard.verdict,
    summary: buildSummary(guard.verdict, rules, aggGates.reason ?? null),
