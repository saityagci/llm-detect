...
const ellipsis_hand_typed = {
  ...
  compute(ctx) {
    ...
    return n > 0 ? { value: n, matched: `${n} hand-typed ellipsis runs` } : null;
  },
};
...
const repeated_punct_emoticon = {
  ...
  compute(ctx) {
    ...
    return n > 0 ? { value: n, matched: `${n} repeated-punct/emoticon hits` } : null;
  },
};
...
const letter_elongation = {
  ...
  compute(ctx) {
    ...
    return n > 0 ? { value: n, matched: `${n} elongation runs` } : null;
  },
};
...
const multi_exclam = {
  ...
  compute(ctx) {
    ...
    return n > 0 ? { value: n, matched: `${n} runs of "!!"` } : null;
  },
};
...
const wa_single_asterisk = {
  ...
  compute(ctx) {
    const m = ctx.raw.match(/(?<!\*)\*[^*\n]{2,60}\*(?!\*)/u);
    return m ? { value: 1, matched: m[0] } : null;
  },
};
...
const emoji_bullet_led = {
  ...
  compute(ctx) {
    const n = ctx.linesAll.filter((l) => /^\s*\p{RGI_Emoji}\s+\p{Lu}/v.test(l)).length;
    return n >= 2 ? { value: 1, matched: `${n} emoji-led section lines` } : null;
  },
};
...
const tr_bare_capital_I = {
  ...
  compute(ctx) {
    const n = ctx.tokens.filter((t) => /^I(?=[a-zçğıöşü])/u.test(t.t)).length;
    return n > 0 ? { value: n, matched: `${n} bare capital I` } : null;
  },
};
...
const tr_chat_morphology = {
  ...
  compute(ctx) {
    ...
    const n = fut + wrd;
    return n > 0 ? { value: n, matched: `${fut} reduced futures + ${wrd} chat words` } : null;
  },
};
...
const tr_formal_copula = {
  ...
  compute(ctx) {
    ...
    const n = ctx.tokens.filter((t) => {
      const low = t.t.toLocaleLowerCase('tr').replace(/i̇/gu, 'i');
      return /(makta|mekte)(dır|dir)$/u.test(low) || /(malı|meli)dır$/u.test(low);
    }).length;
    return { value: (n * 100) / ctx.tokenCount, matched: `${n} -mAktAdIr/-mAlIdIr per ${ctx.tokenCount} tokens` };
  },
};
...
const bold_lead_in_list = {
  ...
  compute(ctx) {
    ...
    const boldLead = /^\s*[-*•\p{Nd}.)]+\s*\*\*[^*\n]{2,40}\*\*\s*[:：-]/u;
    const boldLeadInner = /^\s*[-*•\p{Nd}.)]+\s*\*\*[^*\n]{2,40}:\s*\*\*/u;
    const plainLead = /^\s*[-*•]\s*\p{Lu}[^:\n]{2,40}:\s+\S/u;
    const hits = ctx.linesAll.filter((l) => boldLead.test(l) || boldLeadInner.test(l) || plainLead.test(l));
    return hits.length > 0 ? { value: hits.length, matched: hits[0].trim().slice(0, 60) } : null;
  },
};
...
const colon_led_list = {
  ...
  compute(ctx) {
    ...
    for (let i = 0; i < ls.length - 1; i++) {
      if (/:\s*$/u.test(ls[i]) && /^\s*[-*•\p{Nd}]/u.test(ls[i + 1])) n++;
    }
    return n > 0 ? { value: n, matched: `${n} colon-led lists` } : null;
  },
};
...
const enumerated_openers = {
  ...
  compute(ctx) {
    ...
    return best >= 1 && hits.length >= 3 ? { value: 1, matched: hits.slice(0, 3).join(' | ') } : null;
  },
};
...
const balanced_contrast_frame = {
  ...
  compute(ctx) {
    ...
    return n > 0 ? { value: n, matched: first } : null;
  },
};
...
const greeting_signoff_frame = {
  ...
  compute(ctx) {
    ...
    const hasOpener = o.test(first);
    const sm = s.exec(ctx.foldedLex);
    if (!(hasOpener && sm)) return null;
...
const hedge_density = {
  ...
  compute(ctx) {
    ...
    if (pats.length === 0) return null;
    let n = 0;
    for (const p of pats) n += count(ctx.foldedLex, p);
    return { value: (n * 100) / ctx.tokenCount, matched: `${n} hedges per ${ctx.tokenCount} tokens` };
  },
};
...
const contraction_apostrophe_drop = {
  ...
  compute(ctx) {
    const hits = ctx.tokens.map((t) => t.t.toLowerCase()).filter((t) => CONTRACTIONS.has(t));
    return hits.length > 0 ? { value: hits.length, matched: [...new Set(hits)].slice(0, 5).join(', ') } : null;
  },
};
...
const self_correction_marker = {
  ...
  compute(ctx) {
    ...
    for (const p of SELF_CORRECT) {
      ...
      if (m) { n++; first ??= m[0].trim().slice(0, 30); }
    }
    return n > 0 ? { value: n, matched: first } : null;
  },
};
