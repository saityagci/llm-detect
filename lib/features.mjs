...
const bold_lead_in_list = {
  ...
  compute(ctx) {
    ...
    const hits = ctx.linesAll.filter((l) => BOLD_LEAD.test(l) || BOLD_LEAD_INNER.test(l));
...
const colon_led_list = {
  ...
  compute(ctx) {
    ...
    for (let i = 0; i < ls.length - 1; i++) {
      if (/:\s*$/u.test(ls[i]) && /^\s*[-*•\p{Nd}]/u.test(ls[i + 1])) { n++; first ??= ls[i].trim().slice(0, 60); }
    }
    ...
    for (const l of ls) {
      if (PLAIN_LEAD.test(l) && !BOLD_LEAD.test(l) && !BOLD_LEAD_INNER.test(l)) {
        n++; first ??= l.trim().slice(0, 60);
      }
    }
    return n > 0 ? { value: n, matched: first ?? `${n} colon-led lists` } : null;
  },
};
