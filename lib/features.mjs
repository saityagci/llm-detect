...
/**
 * R50(c) — FUNCTION-WORD SENTENCE STARTERS.
 * `parallel_openers` measured how often two sentences begin with the same first token. Ordinary
 * narrative English begins sentences with "The", "I", "It", "We" all day, so the feature was
 * counting grammar, not habit: it carried two personal narratives and two ESL essays to
 * `ai_style_indicators`. These starters are removed from the uniqueness computation, so what is
 * left is repeated CONTENT openers and the connector set. The connector bonus is unchanged.
 */
const FUNCTION_OPENERS = {
  en: new Set(['the', 'a', 'an', 'i', 'it', 'this', 'that', 'he', 'she', 'they', 'we', 'you',
    'there', 'my', 'his', 'her', 'our', 'in', 'on', 'at', 'when', 'but', 'and', 'so', 'if',
    'as', 'then']),
  tr: new Set(['bu', 'o', 'ben', 'biz', 'bir', 've', 'ama', 'sonra', 'çünkü', 'hem', 'de', 'da']),
};
...
const parallel_openers = {
  ...
  compute(ctx) {
    const ss = ctx.sentences;
    if (ss.length < 6) return null;
    const fold = (t) => caseFold(t ?? '', ctx.lang);
    const stop = FUNCTION_OPENERS[ctx.lang] ?? FUNCTION_OPENERS.en;
    // R50(c): the uniqueness computation runs over CONTENT openers only.
    const contentFirsts = ss.map((s) => fold(s.tokens[0])).filter((t) => t !== '' && !stop.has(t));
    const total = ss.length;
    const conns = CONNECTORS[ctx.lang] ?? [];
    const connHits = connectorHits(ctx.foldedLex, conns);
    const uniq = new Set(contentFirsts).size;
    const repeated = contentFirsts.length - uniq;
    // The VALUE keeps its old scale — the share of repeats among the openers being counted — so
    // the cell's mu/sigma (0.25 / 0.15) still mean what they meant. Only WHICH openers are
    // counted changed. With fewer than two content openers there is nothing to measure and the
    // feature returns null rather than a fabricated 0 (SPEC C.3 invariant 4).
    let value = contentFirsts.length >= 2 ? 1 - uniq / contentFirsts.length : 0;
    if (connHits >= 3) value = Math.min(1, value + 0.15);
    if (contentFirsts.length < 2 && connHits < 3) return null;
    return {
      value,
      matched: `${repeated} of ${total} sentences open with the same content word; `
        + `${connHits} connector openers`,
      spans: [],
    };
  },
};
