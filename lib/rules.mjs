...
/**
 * R51(b)/R52: a corpus row may name its author as `sender` or as `student` — the documented class
 * file used `student`, which left the different-sender guard below inoperative on the documented
 * path (a same-student pair sat at Jaccard 0.76, just under the threshold, by luck). `sender`
 * wins when a row carries both.
 */
export function authorOf(row) {
  return row.sender ?? row.student ?? null;
}

export function buildCorpusIndex(docs) {
  return docs.map((d) => ({
    id: d.id, sender: authorOf(d), shingles: shingles((d.text ?? '').normalize('NFC').toLowerCase()),
