...
export function decideVerdict(p) {
  ...
  if (verdict === 'leaning_llm') {
    ...
    if (!(nonProxy.length >= 2 && nonProxyGroups.size >= 2)) {
      ...
      // R50(d): the warning has TWO cases and a reader of an evaded assistant essay (LLM channel
      // 0.54, carried by rhythm alone) must not be told its proxies were the issue.
      const headline = nonProxy.length === 0
        ? `register proxies only — ${proxies.map((s) => s.id ?? s.name).sort().join(', ') || '(none)'}`
        : `material non-proxy evidence from one group only (${[...nonProxyGroups].sort().join(', ')}) — `
          + `${nonProxy.map((s) => s.id ?? s.name).sort().join(', ')}`
          + `, beside the register proxies ${proxies.map((s) => s.id ?? s.name).sort().join(', ') || '(none)'}`;
