...
in CAVEATS only, sourced.

`stylometry.mjs` and `RUBRIC.md` live at `$LLM_DETECT_HOME`; fall back to `$HOME/Desktop/llm-detect`
only when **unset**. Set but unresolvable means **missing**, no second place: say so on the CLI
line, judge alone, cap `leaning_*`.

## Procedure

1. **Normalize.** Inline text → a scratchpad file; `.jsonl` → batch; any other path is one document.
   Never write in a git repo unless told.
2. **Judge first, and write it down first.** Read `RUBRIC.md` (§9 is your flag reference) and the
   ...
   **to a file before invoking the CLI**; write it once, never reopen it, post-CLI notes to a second
   file. RUBRIC's hard evidence rule binds: no quote, not present.
3. **Run the CLI.**
   `node "$LLM_DETECT_HOME/stylometry.mjs" --json --file <path> --allow-uncalibrated <flags>`
   Map statements to flags **per RUBRIC §9**: pass a flag only when the caller states the thing it
   encodes, never inferred. "Essay"/"exam answer" ⇒ `--preset essay`; a priors path ⇒
...
4. **Read the CLI JSON.** A rule is a matched artifact, a signal a weak prior — never merge them.
   RUBRIC's table, invariants and §8 bind you: the table, not intuition — it lists the asymmetric
   cells.
5. **Report** in RUBRIC §5's skeleton, every line of it, plain — never in code fences. `VERDICT:` is
   line 1: no preamble. `LABEL:` next, `summary.label` verbatim. Working files last, on one optional
   `NOTES:` line or nowhere. The CLI line copies the flags **actually passed** plus the gate
   reason on `insufficient_text` (`below_char_floor` ≠ `too_few_active_features`). `[CONFLICT]` only
   in the six ⚠ cells RUBRIC lists — `EH` × `EL` untagged — and never shift toward human on one: the
...
6. **Batch (`.jsonl`).** One CLI pass. Judge only `uncertain` / `leaning_*` rows plus a 10% audit of
   confident ones, capped at 40; over the cap take the 40 nearest the boundary, saying how many you
   skipped. Group by `sender`.
